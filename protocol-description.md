...
* <polling_period_ms> - период отправки значений датчика в [непрерывном](#о-режиме-непрерывного-опроса) режиме. Формат - целочисленный, в миллисекундах, от 0 и выше. Пример - '500'.  
Рассмотрим подробнее команды протокола:
***
#### ***AT***
Запрос наличия и работоспособности MODULE - классическая АТ-команда ***АТ***. MODULE должен вернуть ***OK***
***
#### ***AT+CHECK***
Проверка состояния готовности MODULE.  
* в варианте Test (***AT+CHECK=?***) MODULE должен вернуть ***OK***
...
* в варианте Test ('AT+READ=?') MODULE должен вернуть ***OK***
* в варианте Write с одним параметром (AT+CFG=<index>) MODULE должен вернуть:
