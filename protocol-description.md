...
8. [О управлении потоком RTS/CTS](#о-управлении-потоком-rtscts)
...
* **\[sensor_params\]** - список из двух и более параметров, разделённых запятой. Пример: **5,3.14,"text"**
***
...
|2 |[AT+CHECK](#atcheck) |Test, Read        |Проверка готовности MODULE к работе          |
...
|5 |[AT+READ](#atread)   |Test, Write       |Одиночное чтение датчика                     |
...
#### ***AT***
Запрос наличия и работоспособности MODULE - классическая АТ-команда 'АТ'. MODULE должен вернуть  
> OK
***
#### ***AT+CHECK***
Проверка состояния готовности MODULE.  
* в варианте Test ('AT+CHECK=?') MODULE должен вернуть  
    > OK
* в варианте Read ('AT+CHECK?') MODULE должен вернуть состояние своей готовности:
    * если MODULE пока не готов работать  
    > AT+CHECK:"BUSY"  
    > OK
    * если MODULE готов работать  
    >AT+CHECK:"READY"  
    >OK
***
#### ***AT+LIST***
Получение ассоциированного списка датчиков \<index:uuid\>.
* в варианте Test ('AT+LIST=?') MODULE должен вернуть  
    > OK
* в варианте Read (AT+LIST?) MODULE должен вернуть список пар в виде:  
    > AT+LIST:index1,uuid1,index2,uuid2,...,indexN,uuidN  
    > OK  
    >   
    > **пример:**  
    > -\> AT+LIST?  
    > \<- AT+LIST:0,"123e4567-e89b-12d3-a456-426655440000",1,"123e4567-e89b-12d3-a456-426655440010"  
    > \<- OK  
***
#### ***AT+CFG***
Работа с параметрами датчиков.
* в варианте Test ('AT+CFG=?') MODULE должен вернуть  
    > OK
* в варианте Read (AT+CFG?) MODULE должен последовательно вернуть параметры всех датчиков (о \[sensor_params\] сказано выше)  
    > AT+CFG:\[sensor1_params\]  
    > AT+CFG:\[sensor2_params\]  
    > ...  
    > AT+CFG:\[sensorN_params\]  
    > OK  

    > **пример:**  
    > -\> AT+CFG?  
    > \<- AT+CFG:0,"PLOTTER",0,0  
    > \<- AT+CFG:1,"PLOTTER",5,0  
    > \<- OK
* в варианте Write с одним параметром (AT+CFG=<index>) MODULE должен вернуть:
    * если такой индекс найден, то возвращаются параметры датчика по этому индексу
    > AT+CFG:\[sensor_params\]  
    > OK
    * если такой индекс отсутствует, то
    > ERROR
* в варианте Write с полным набором параметров (AT+CFG=\[sensor_params\]) MODULE должен вернуть:
    * если такой индекс найден и параметры адекватны, то датчику присваиваются новые параметры и возвращается
    > OK
    * если такой индекс отсутствует, то
    > ERROR
***
...
* в варианте Test ('AT+READ=?') MODULE должен вернуть  
    > OK
* в варианте Write с одним параметром (AT+CFG=<index>) MODULE должен вернуть:
    * если такой индекс найден, то возвращаются данные датчика по этому индексу в предустановленном формате
    > [data in special format]  
    > OK
    * если такой индекс отсутствует, то
    > ERROR
***
...
* "PLOTTER" - данный формат передаёт значения с плавающей точкой в виде ASCII-символов. Сообщение выглядит следующим образом - '$<index>,<value1>,<value2,...,<valueN>\r\n', где <index> - это индекс датчика, <valueX> - это данные канала датчика. Данные каналов передаются в порядке, соответствующем их субиндексу, определённому в спецификации. Пример для многоканального датчика: ***$2,5.85,10.0\r\n*** - это датчик с индексом 2, данные нулевого канала - 5.85, данные первого канала - 10.0. Пример для однокнального датчика: ***$2,5.85\r\n*** - это датчик с индексом 2, данные канала - 5.85.
***
## О режиме непрерывного опроса
Датчик, который в параметрах конфигурации получил значение <polling_period_ms>, должен начать передачу своих [данных](#протокол-данных) с периодом, соответствующим полученной уставке. Отменить этот режим можно передав значение <polling_period_ms> равное 0 или сбросив MODULE.
***
