...
|3 |AT+LIST               |Test, Read        |Чтение пар \<index-UUID\> доступных датчиков |
|4 |AT+CFG                |Test, Read, Write |Чтение/установка параметров датчиков         |
|5 |AT+READ               |Test, Write       |Одиночное чтение датчика                     |
...
Проверка состояния готовности MODULE.  
    ...
    * в варианте Read ('AT+CHECK?') MODULE должен вернуть состояние своей готовности:
        * 'AT+CHECK:"BUSY"' 'OK' - если MODULE пока не готов работать;
        * 'AT+CHECK:"READY"' 'OK' - если MODULE готов работать;
#### AT+CFG - параметры датчика(-ов) (Sensor ConFiGuration). Данная команда представлена в трёх типах: Test, Read и Set.
