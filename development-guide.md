...
Далее необходимо на основе этих таблиц составить json-спецификации. За основу следует брать [json-схему](/specifications/schema/sensor.schema.json). Следует обратить внимание, что **ВАЖНОЕ** значение имеет порядок следования диапазонов датчика - этот порядок явялется индексом при обрщении блока MASTER к конкретному диапазону датчика.
